...
describe("Unit AllCreditsCacheHandler tests", () => {
    const testData = [
        {
            name: "get credits from canister",
            input: {
        ...
        {
            name: "get credits from cache, cache is empty",
            input: {
                servicePrincipal: "pmr6h-yaaaa-aaaao-a3myq-cai",
                loadType: LoadType.Cache
            },
    ...
    for (let test of testData) {
        it(test.name, async () => {
            ...
            const allCreditsHandler = new AllCreditsCacheHandler(logger, identifierService, cacheRepository);
