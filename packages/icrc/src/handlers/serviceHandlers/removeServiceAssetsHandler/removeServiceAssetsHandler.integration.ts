...
describe("Unit AddServiceHandler tests", () => {
    const testData = [
        {
            name: "add service",
            input: {
                ledgerAddress: "asset-test-test-test-test",
    ...
    for (let test of testData) {
        it(test.name, async () => {
            ...
            serviceRepository.removeServiceAsset = test.data.isSuccess
                ? jest.fn().mockReturnValue(Promise.resolve(undefined))
                : jest.fn().mockReturnValue(Promise.reject("Test Error"));
            const logger = new MockLogger();
            const removeServiceHandler = new RemoveServiceAssetsHandler(logger, serviceRepository);
