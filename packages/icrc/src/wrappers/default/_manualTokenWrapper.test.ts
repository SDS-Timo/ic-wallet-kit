import { MockLogger } from "@icrc/__tests_utils/mockLogger";
import { mockAnonymousIdentifierService } from "@icrc/__tests_utils/seedToIdentity";
import { ManualTokenWrapper } from "@icrc/wrappers/default/manualTokenWrapper";
import { LedgerWrapper } from "@icrc/wrappers/icrc/ledgerWrapper/ledgerWrapper";

describe("ManualTokenWrapper Tests", () => {
    let mockLogger = new MockLogger();
    let mockIdentifierService = mockAnonymousIdentifierService();

    beforeEach(() => {

        jest.clearAllMocks();
    });

    it("ManualTokenWrapper: getTokens returns token list successfully", async () => {
        const mockMetadata = {
            decimals: 8,
            logo: "mock-logo",
            name: "Mock Token",
            symbol: "MOCK",
        };


        const ledgerWrapper = new (<new () => LedgerWrapper><unknown>LedgerWrapper)() as jest.Mocked<LedgerWrapper>;
        LedgerWrapper.create = jest.fn().mockReturnValue(ledgerWrapper);
        ledgerWrapper.getIcrcMetadataInfo = jest.fn().mockResolvedValue(mockMetadata);

        const wrapper = new ManualTokenWrapper(mockLogger, mockIdentifierService);
        const result = await wrapper.getTokens();

        expect(result).toEqual([
            {
                ledgerAddress: "6c7su-kiaaa-aaaar-qaira-cai",
                decimal: mockMetadata.decimals,
                indexAddress: "oo6x4-xiaaa-aaaap-abrza-cai",
                logo: mockMetadata.logo,
                name: mockMetadata.name,
                symbol: mockMetadata.symbol,
            },
            {
                ledgerAddress: "jwcfb-hyaaa-aaaaj-aac4q-cai",
                decimal: mockMetadata.decimals,
                indexAddress: "",
                logo: "data:image/png;base64,",
                name: "Origyn Legacy",
                symbol: "OGYL",
            },
        ]);
    });

    it("ManualTokenWrapper: getTokens handles fetch error gracefully", async () => {

        const ledgerWrapper = new (<new () => LedgerWrapper><unknown>LedgerWrapper)() as jest.Mocked<LedgerWrapper>;
        LedgerWrapper.create = jest.fn().mockReturnValue(ledgerWrapper);
        ledgerWrapper.getIcrcMetadataInfo = jest.fn().mockRejectedValue(new Error("Mock error"));

        mockLogger.logError = jest.fn();

        const wrapper = new ManualTokenWrapper(mockLogger, mockIdentifierService);
        const result = await wrapper.getTokens();

        expect(result).toEqual([]);

        expect(mockLogger.logError).toHaveBeenCalledWith(new Error("Mock error"), "Manual wrapper error");

    });
});
