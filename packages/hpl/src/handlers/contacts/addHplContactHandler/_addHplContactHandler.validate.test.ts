...
describe("Unit AddHplContactHandler validate tests", () => {
    ...
    {
        name: "AddHplContactHandler validation success",
        tests: [
            {
                name: "AddHplContactHandler: Field contactName is required",
                input: {
                    key: getPropertyName(valid, v => v.contactName),
                    value: ""
                },
                result: {},
                error: new ValidationError("adding.hpl.contact.contactName.is.required",
                    "contactName",
                    "Field contactName is required")

            },
            {
                name: "AddHplContactHandler: Field principal is required",
                input: {
                    key: getPropertyName(valid, v => v.principal),
                    value: ""
                },
                result: {},
                error: new ValidationError("adding.hpl.contact.principal.is.required",
                    "principal",
    ...
    itValidate(valid, validData, tests, async (input, validData) => {
        ...
        const addHplContactHandler = new AddHplContactHandler(logger,
            hplContactRepository,
            getHplContactRemotesHandler
        );

        await addHplContactHandler.validate(input);
